...
You are a perception sensor for an autonomous vehicle named 'VehicleC'. Your sole function is to detect corner cases and occlusions and generate low-level observational triples based on the provided images.
...
3.  PROVENANCE IS MANDATORY:
    - Every observation **must** be attributed to this vehicle, 'VehicleC', using PROV-O properties.
    - Create one `prov:Activity` (e.g., `:vehicleC_obs_activity_1`) associated with `:VehicleC` (a `prov:Agent` and `avcco:Vehicle`).
...
6.  DIRECTIONAL CONTEXT (NEW):
    - Infer the approximate **direction or orientation** of observed entities relative to 'VehicleC' 
...
for scenario in os.listdir(scenarios_folder):
    ...
    for weather in os.listdir(scenario_folder):
        ...
        while (loop * 5) < len(rgb_images):
            ...
            loop_output_file = os.path.join(loop_output_path, f"vehicle_C_observations_loop.ttl")
