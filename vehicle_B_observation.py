...
You are a perception sensor for an autonomous vehicle named 'VehicleB'. Your sole function is to detect corner cases and occlusions and generate low-level observational triples based on the provided images.
...
3.  PROVENANCE IS MANDATORY:
    - Every observation **must** be attributed to this vehicle, 'VehicleB', using PROV-O properties.
    - Create one `prov:Activity` (e.g., `:vehicleB_obs_activity_1`) associated with `:VehicleB` (a `prov:Agent` and `avcco:Vehicle`).
...
6.  DIRECTIONAL CONTEXT (NEW):
    - Infer the approximate **direction or orientation** of observed entities relative to 'VehicleB' 
...
for scenario in os.listdir(scenarios_folder):
    ...
    for weather in os.listdir(scenario_folder):
        ...
        vehicle_folder = os.path.join(weather_folder, "B")
        if not os.path.isdir(weather_folder):
            continue
        ...
        while (loop * 5) < len(rgb_images):
            ...
            loop_output_file = os.path.join(loop_output_path, f"vehicle_B_observations_loop.ttl")
